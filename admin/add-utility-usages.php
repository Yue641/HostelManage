...
$utilities_query = "SELECT id, name, price_per_unit FROM utilities";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $("select[name='utility_id'], input[name='usage_amount']").on("change keyup", function(){
            ...
            var usage_amount = $("input[name='usage_amount']").val();
            
            if (utility_id && usage_amount) {
                $.ajax({
                    type: "POST",
                    url: "check-availability-admin.php", // File xử lý lấy giá
                    data: { utility_id: utility_id },
                    success: function(response){
                        var price_per_unit = parseFloat(response);
                        if (!isNaN(price_per_unit)) {
                            var total = usage_amount * price_per_unit;
