<!-- Sidebar navigation-->
<nav class="sidebar-nav">

    <ul id="sidebarnav">
    
        <li class="sidebar-item"> <a class="sidebar-link sidebar-link" href="dashboard.php"
        aria-expanded="false"><i data-feather="home" class="feather-icon"></i><span
         class="hide-menu">Dashboard</span></a></li>

        <li class="list-divider"></li>

        <li class="nav-small-cap"><span class="hide-menu">Features</span></li>
                            
        <li class="sidebar-item"> <a class="sidebar-link sidebar-link" href="request.php"
        aria-expanded="false"><i class="fas fa-h-square"></i><span
        class="hide-menu">Request</span></a></li>

        <li class="sidebar-item"> <a class="sidebar-link sidebar-link" href="check-bills.php"
        aria-expanded="false"><i class="fas fa-bed"></i><span
        class="hide-menu">Check Bills</span></a></li>

                           
    </ul>
</nav>
<!-- End Sidebar navigation -->